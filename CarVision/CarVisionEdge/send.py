...
receive = None
started = False
data_start = b"\x07\x23"
response = None
data_end = b"\xC8"
data_error = b"\xFF\xFF\xC8\xFF"
...
def main() -> None:
    ...
    while True:
        ...
        if len(data) == 4 and data[0:2] != b"\x07\x23" and data[3] != 0xC8:
            ser.write(b"\x07\x23\xFF\xFF\xC8\xFF\xC8")
            continue
        ...
        while response == None and count < 100:
            ...
            if receive == 0x02: # 物料颜色检测
                colr, _ = Cdet.run(roi)
                if colr: response = bytes([colr])

            if receive == 0x03: # 绿色圆环坐标
                cposi = Fdet.find(frame)
                if cposi:
                    x, y = cposi
                    x_h = (x & 0xFF00) >> 8
                    x_l = x & 0x00FF
